...
class FreelancerWithPrinter(Printer, Freelancer):
    pass
...
def main() -> None:
    henry = HourlyEmployee(name="Henry", id=12345, pay_rate=5000, hours_worked=100)
    print(f"{henry.name} earned ${(henry.compute_pay() / 100):.2f}.")

    sarah = SalariedEmployeeWithCommission(name="Sarah", id=47832, monthly_salary=500000, contracts_landed=10)
    print(f"{sarah.name} earned ${(sarah.compute_pay() / 100):.2f}.")

    rommel = FreelancerWithPrinter(name="Rommel", id=123, pay_rate=6000, hours_worked=100)
    rommel.print_pay()
